...
//	String scoreMsg = "";
	...
	public void draw() {
		...
		for (int i = 0; i < labels.size(); i++) {
			if (labels.get(i).equals("palm")) {
				text("hit", xCors.get(i), yCor);
			} else {
				text(labels.get(i), xCors.get(i), yCor);
			}
			
			xCors.set(i,  xCors.get(i) - 2);
		...
		if(classifier != null) {
			...
			if (classNames[classIndex].equals("palm")) {
				text("hit", 20, 30);
			} else {
		...
		if (printLabel.equals("palm")) {
			text("Classified as: hit", 10, 30);
		} else {
